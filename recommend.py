...
def give_rating(profile, lang_to_rate):
    raw_ratings =  sum([float(sum([lang_to_rate.get(key) != other.get(key) for key, val in lang_to_rate.iteritems() if key not in ["name", "description"]])) #List of booleans corresponding to differences
...
#print give_recommendations({"name" : "flerpus", "skills" : {"python": 2, "java" : 4, "clojure" : 8}, "exceptions" : ["c", "haskell"]}, {"vm": "jvm", "game" : "y"})
